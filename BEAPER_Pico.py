...
# Raspberry Pi Pico analog inputs
ADC0 = ADC(Pin(ADC0_PIN))
ADC1 = ADC(Pin(ADC1_PIN))
ADC2 = ADC(Pin(ADC2_PIN))
VSYS = ADC(Pin(29))   # Pico voltage regulator input (VSYS / 3)
MCU_TEMP = ADC(ADC.CORE_TEMP)  # RP2xxxx die temperator sensor
...
def RV1_level():
    # Read raw potentiometer RV1 value. Set JP2 to Enviro.
    return ADC1.read_u16()  # Clockwise -> higher values

def RV2_level():
    # Read raw potentiometer RV2 value. Set JP3 to Enviro.
    return ADC2.read_u16()  # Clockwise -> higher values

def VSYS_volts():
    # Read converted system voltage in volts.
...
H8_PIN = LS1_PIN    # Shared with piezo speaker pin
